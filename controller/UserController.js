...
const UserController = {
  ...
  getUserList: async (req, res, next) => {
    try {
      ...
      result.rows.forEach((user) => {
        user.roleIds = roles.filter((role) => role.user_id === user.id).map((role) => role.role_id)
      })
      if (!result) {
  ...
  changeUserStatus: async (req,res,next) => {
    try {
      const result = await UserService.updateUserStatus(req.body.status,req.params.id)
      if (!result) {
