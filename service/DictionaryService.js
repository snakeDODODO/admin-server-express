...
const DictionaryService = {
  ...
  // 获取数据字典存储store
  getDictionaryForStore: async () => {
    try {
      const result = await DictionaryModule.findAll({
        include: [
          {
            model: DictionaryDataModule,
            as: 'data',
            // 加这个可以分组 || group也可以
            attributes: ['id', 'dictionaryname', 'dictionaryvalue', 'status', 'dictionary_id']
          }
        ],
        raw: false
      });
